            ...
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if ((OSPrioHighRdy != OSPrioCur)&&(OSTCBCur->ExecTime!=0)) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    if(OSTCBCur->OSTCBId<62) {
                        //printf("\t%d\tPreempte\tTask(%d)(%d)\tTask(%d)(%d)\t\n",OSTimeGet(),OSTCBCur->OSTCBId,OSTCBCur->NoJob,OSTCBHighRdy->OSTCBId,OSTCBHighRdy->NoJob);
                    }
                    else {
    ...
    if (OSRunning == OS_TRUE) {
...
#endif
        int min_deadline = OSTCBCur->Deadline;
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            ...
            if (ptcb->OSTCBDly != 0) {                     /* No, Delayed or waiting for event with TO     */
                ...
                if(ptcb->Deadline < min_deadline && ptcb->OSTCBDly == 0){   // if higher prio ready, need to inherit deadline
        ...
        if(OSTCBCur->Deadline != min_deadline && OSTCBCur->GetRes == 1){
            ...
            printf("\t%d\tTask_%d inherit deadline\t\tdeadline %d\n",OSTimeGet(),OSTCBCur->OSTCBId-10, OSTCBCur->Deadline);
        }
        //printf("%d\n",OSTime);
        OSTCBCur->ExecTime--;
...
static  void  OS_InitEventList (void)
{
    ...
    pevent1->chain                  = 0; // pend multi-resources ?
#if OS_EVENT_NAME_SIZE > 1
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            ...
            if(OSTimeGet() > OSTCBCur->Deadline)
                printf("\t%d\tMiss Deadline\tTask(%d)(%d)\n",OSTimeGet(),OSTCBCur->OSTCBId,OSTCBCur->NoJob);
            
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];

                //printf("\t%d\tComplete\tTask(%d)(%d)\tTask(%d)(%d)\t%d\n",OSTimeGet(),OSTCBCur->OSTCBId,OSTCBCur->NoJob,OSTCBHighRdy->OSTCBId,OSTCBHighRdy->NoJob,(OSTimeGet()-OSTCBCur->Start));
                /*OSTCBCur->NoJob += 1;
...
static  void  OS_SchedNew (void)
{
    ...
    while(i < OS_LOWEST_PRIO) {                                   // search all priority table 
        if(OSTCBPrioTbl[i]!=(OS_TCB*)0 && OSTCBPrioTbl[i]!=OS_TCB_RESERVED) {  // not an empty TCB
            ...
            if(OSTCBPrioTbl[i]->OSTCBDly==0 && (OSTCBPrioTbl[i]->Preempt_lv > Ceiling || OSTCBCur->OSTCBPrio==i)) {                                 // ready task
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt, INT32U deadline)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        ptcb->Org_Deadline = deadline;
        if(ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO){
            ptcb->Preempt_lv = MAX_Preempt_lv - (prio-11);      // 1~3, 3 is max
            ptcb->GetRes = 0;
        }
        else
            ptcb->Preempt_lv = 0;
