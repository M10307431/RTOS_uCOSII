...
void  OSIntExit (void)
{
    ...
    //OSTCBCur->Resp = OSTimeGet() - OSTCBCur->Start;
    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                OS_SchedNew();
                if ((OSPrioHighRdy != OSPrioCur)&&(OSTCBCur->ExecTime!=0)) {          /* No Ctx Sw if current task is highest rdy */
                    OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy];
                    if(OSTCBCur->OSTCBId<62) {
                        printf("\t%d\tPreempte\tTask(%d)(%d)\tTask(%d)(%d)\t\n",OSTimeGet()/SCALE,OSTCBCur->OSTCBId,OSTCBCur->NoJob,OSTCBHighRdy->OSTCBId,OSTCBHighRdy->NoJob);
                    }
                    else {
                        printf("\t%d\tComplete\tTask(%d)(%d)\tTask(%d)(%d)\t\n",OSTimeGet()/SCALE,OSTCBCur->OSTCBId,OSTCBCur->NoJob,OSTCBHighRdy->OSTCBId,OSTCBHighRdy->NoJob);
                        OSTCBCur->NoJob +=1;               /* Plus # of job for static task*/
                    }
                    //printf("%d\n",OSTCBPrioTbl[APTASK_PRIORITY]->Deadline); 
...
void  OSTimeTick (void)
{
    ...
    if(OSTime%100 == 0)
    {
        ...
        if(buttons != NONE_PRESSED && (OSTCBPrioTbl[APTASK_PRIORITY]->Deadline <= OSTime || OSTCBPrioTbl[APTASK_PRIORITY]->Deadline==65535))
        {
            if(led >= 0x80)
                led = 0x01;
            else
            ...
            OSTCBPrioTbl[APTASK_PRIORITY]->Deadline = (OSTime/SCALE+ bp/SCALE*ServerSize_denominator/ServerSize_numerator)*SCALE;
            OSTCBPrioTbl[APTASK_PRIORITY]->OSTCBDly = 0;
            printf("Aperiodic task arrives at time %d", OSTime/SCALE);
            printf(" the computation time is %d,", bp/SCALE);
            printf(" deadline : %d.\n", OSTCBPrioTbl[APTASK_PRIORITY]->Deadline/SCALE);
            //printf("%d\n", OSTCBPrioTbl[APTASK_PRIORITY]->ExecTime/SCALE);
        }
    }
    OS_EXIT_CRITICAL();
    ...
    if (OSRunning == OS_TRUE) {
        ...
        if(OSTimeGet()==1*SCALE){
            ...
            OSTCBPrioTbl[APTASK_PRIORITY]->Deadline = OSTimeGet()+OSTCBPrioTbl[APTASK_PRIORITY]->Exec*8/2;
            OSTCBPrioTbl[APTASK_PRIORITY]->OSTCBDly = 0;
        }
        ...
        */
            ...
            if (ptcb->OSTCBDly != 0) {                     /* No, Delayed or waiting for event with TO     */
                ...
                if (--ptcb->OSTCBDly == 0) {               /* Decrement nbr of ticks to end of delay       */
                    ...
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* Is task suspended?       */
                        ...
                        /*if(ptcb->OSTCBPrio == APTASK_PRIORITY)
                        {
                            ptcb->Deadline = OSTime+bp*ServerSize_denominator/ServerSize_numerator;
                            printf("%d\n", ptcb->Deadline/SCALE);
                        }*/
                    }
                }
                else if(ptcb->Deadline-ptcb->Period == OSTimeGet()){
                    ptcb->OSTCBDly = 0;
                    OSRdyGrp               |= ptcb->OSTCBBitY;             /* No,  Make ready          */
                    OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
                }
        ...
        //printf("%d , %d, %d\n",OSTCBCur->OSTCBPrio,OSTCBCur->ExecTime,OSTCBCur->Deadline);
        //printf("%d , %d, %d\n",OSTCBPrioTbl[3]->OSTCBPrio,OSTCBPrioTbl[3]->ExecTime,OSTCBPrioTbl[3]->Deadline);
        //printf("%d\n",OSTCBPrioTbl[APTASK_PRIORITY]->ExecTime);
...
static  void  OS_InitTaskIdle (void)
{
    ...
    (void)OSTaskCreateExt(OS_TaskIdle,
                          ...
                          65535u);/* Enable stack checking + clear stack  */
    #else
    (void)OSTaskCreateExt(OS_TaskIdle,
                          (void *)0,                                 /* No arguments passed to OS_TaskIdle() */
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
                //printf("%d\n",OSTCBCur->Start);
                printf("\t%d\tComplete\tTask(%d)(%d)\tTask(%d)(%d)\t%d\n",OSTimeGet()/SCALE,OSTCBCur->OSTCBId,OSTCBCur->NoJob,OSTCBHighRdy->OSTCBId,OSTCBHighRdy->NoJob,(OSTimeGet()-OSTCBCur->Start)/SCALE);
                OSTCBCur->NoJob += 1;
            ...
            //else
                //printf("\t%d\tWTF\tTask(%d)(%d)\tTask(%d)(%d)\t%d\n",OSTimeGet(),OSTCBCur->OSTCBId,OSTCBCur->NoJob,OSTCBHighRdy->OSTCBId,OSTCBHighRdy->NoJob,OSTCBCur->Resp);
        }
    }
    OS_EXIT_CRITICAL();
}
    ...
    while(i < OS_LOWEST_PRIO) {                                   // search all priority table 
        if(OSTCBPrioTbl[i]!=(OS_TCB*)0 && OSTCBPrioTbl[i]!=OS_TCB_RESERVED) {  // not an empty TCB
            if(OSTCBPrioTbl[i]->OSTCBDly==0) {                                 // ready task
                ...
                if((OSTCBPrioTbl[i]->Deadline < UrgentDeadline)||(OSTCBPrioTbl[i]->Deadline == UrgentDeadline && OSTCBPrioTbl[UrgentPrio]->Period < OSTCBPrioTbl[i]->Period)) {               // most close deadline
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt, INT32U deadline)
{
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3                                /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0;
