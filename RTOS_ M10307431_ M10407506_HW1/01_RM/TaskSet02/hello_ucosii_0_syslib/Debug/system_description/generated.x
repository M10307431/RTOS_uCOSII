 ...
 *
 * Generated: 2015-11-10 19:15:50.724
 *
 */
...
MEMORY
{
    reset : ORIGIN = 0x0c010000, LENGTH = 32
    onchip_ram : ORIGIN = 0x0c010020, LENGTH = 65504
    ext_flash : ORIGIN = 0x00000000, LENGTH = 67108864
    altmemddr : ORIGIN = 0x08000000, LENGTH = 67108864
}

    /* Define symbols for each memory base-address */
 __alt_mem_onchip_ram = 0x0c010000 ;
 __alt_mem_ext_flash = 0x00000000 ;
 __alt_mem_altmemddr = 0x08000000 ;
...
SECTIONS
{
    ...
    .text :
    {
        ...
        *(.dynamic)
        ...
        KEEP (*(.jcr))
        . = ALIGN(32 / 8);
    } >  altmemddr =0x3a880100 /* NOP on Nios2 (big endian) */
    ...
    .altmemddr :
    {
        PROVIDE (_alt_partition_altmemddr_start = ABSOLUTE(.));
        *(.altmemddr .altmemddr.*)
        . = ALIGN(32 / 8);
        PROVIDE (_alt_partition_altmemddr_end = ABSOLUTE(.));
        _end = ABSOLUTE(.);
        end = ABSOLUTE(.);
        __alt_stack_base = ABSOLUTE(.);
    } > altmemddr

    PROVIDE (_alt_partition_altmemddr_load_addr = LOADADDR(.altmemddr));
...
PROVIDE( __alt_heap_limit    = 0x0c000000 );
