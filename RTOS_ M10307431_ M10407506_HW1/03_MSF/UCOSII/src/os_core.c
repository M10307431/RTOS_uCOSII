...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            ...
            //printf("%d %d, ",ptcb->ExecTime,ptcb->Stress);
        ...
        /*  Stress = Remaining Exec / Remaining Time  (100 -> scale)  */
        //OSTCBCur->Stress = (100*OSTCBCur->ExecTime) / (OSTCBCur->Deadline - OSTimeGet());
        //printf("%d , %d, %d, %d\n",OSTCBCur->OSTCBPrio,OSTCBCur->ExecTime,OSTCBCur->Deadline, OSTCBCur->Stress);
...
void  OS_Sched (void)
{
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            OS_SchedNew();
            if(OSTimeGet() >= OSTCBCur->Deadline && OSTCBCur->ExecTime!=0)
    ...
    while(i < OS_LOWEST_PRIO) {                                   // search all priority table 
        if(OSTCBPrioTbl[i]!=(OS_TCB*)0 && OSTCBPrioTbl[i]!=OS_TCB_RESERVED) {  // not an empty TCB
            if(OSTCBPrioTbl[i]->OSTCBDly==0) {                                 // ready task
                if((OSTCBPrioTbl[i]->Stress > UrgentStr)||(OSTCBPrioTbl[i]->Stress == UrgentStr && UrgentPrio > OSTCBPrioTbl[i]->OSTCBPrio)) {               // most close deadline
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt, void *p_arg)
{
    OS_TCB    *ptcb;
    task *t = p_arg;
#if OS_CRITICAL_METHOD == 3                                /* Allocate storage for CPU status register */
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
        //ptcb->Deadline = 65534u;                           // Deadline Initialization
